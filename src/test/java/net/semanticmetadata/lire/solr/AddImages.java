    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for (int j = 1; j < 3; j++) {
            ...
            Thread t = new Thread() {
                ...
                public void run() {
                    try {
                        List<File> files = FileUtils.getAllImageFiles(new File("testdata/small/" + tz), true);
                        int count = 0;
                        BufferedWriter br = new BufferedWriter(new FileWriter("testdata/small/add-us-" + tz + ".xml", false));
                        br.write("<add>\n");
...
//        result.append("<add>\n");
        ...
        getFields(img, result, new LuminanceLayout(), "ll_hi", "ll_ha");
        getFields(img, result, new MSERFeature(), "msr_ce_hi", "msr_ce_ha");
    ...
    private static void getFields(BufferedImage img, StringBuilder result, LireFeature feature, String histogramField, String hashesField) {
        ...
        result.append(Base64.encodeBase64String(feature.getByteArrayRepresentation()).trim());
        result.append("</field>\n");
        result.append("\t\t<field name=\"" + hashesField + "\">");
        result.append(SerializationUtils.arrayToString(BitSampling.generateHashes(feature.getDoubleHistogram())).trim());
        result.append("</field>\n");
    }
